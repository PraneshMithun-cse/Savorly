...
(function () {
  ...
  // Firebase config for self-initialization
  var firebaseConfig = {
  ...
  function waitForFirebase(callback) {
    ...
    setTimeout(function () { waitForFirebase(callback); }, 150);
  }
  ...
  waitForFirebase(function () {
    ...
    const pre_user = JSON.parse(localStorage.getItem('savourlyUser') || 'null');
    if (pre_user) {
      replaceLoginWithProfile(pre_user);
    }

    auth.onAuthStateChanged(function (user) {
      if (user) {
        // User is logged in — ensure localStorage is synced
        ...
        // User is logged out — clear storage and restore UI
    ...
    function createProfileDropdown(user, variant) {
      ...
      dropdown.innerHTML = `
        ...
        <a href="/orders" class="user-dropdown-item">

...
const init = () => {
    ...
    locationTrigger?.addEventListener('click', async () => {
        ...
        // Load addresses from DB if logged in, merge with localStorage
        const user = JSON.parse(localStorage.getItem('savourlyUser') || '{}');
        if (user.firebaseUid) {
            try {
                const res = await fetch(`/api/user/addresses?uid=${user.firebaseUid}`);
                if (res.ok) {
                    const data = await res.json();
                    const dbAddresses = (data.addresses || []).map(a => ({
                        ...a,
                        id: a._id || a.id
                    }));
                    // Merge: DB addresses take priority, add any localStorage-only ones
                    const dbIds = new Set(dbAddresses.map(a => String(a._id || a.id)));
                    const localOnly = savedAddresses.filter(a => !dbIds.has(String(a._id || a.id)));
                    savedAddresses = [...dbAddresses, ...localOnly];
                    localStorage.setItem('savourlyAddresses', JSON.stringify(savedAddresses));
                }
            } catch (err) {
                console.error('Failed to fetch DB addresses:', err);
            }
        }
    ...
    // Detect current location - fast with timeout
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    try {
                        ...
                        // Auto-select GPS location if no address is saved yet
                        const existingAddresses = JSON.parse(localStorage.getItem('savourlyAddresses') || '[]');
                        const existingSelected = localStorage.getItem('savourlySelectedLocation');
                        if (existingAddresses.length === 0 && !existingSelected && locationText) {
                            const city = window.currentGPSAddress.city;
    ...
    useCurrentLocation?.addEventListener('click', () => {
        if (window.currentGPSAddress) {
            ...
            // Auto-retry GPS detection instead of blocking alert
            currentLocationText.textContent = 'Detecting...';
            detectCurrentLocation();
            // Wait and retry
            let retries = 0;
            const checkGPS = setInterval(() => {
                ...
                if (window.currentGPSAddress) {
                    ...
                    const detectedText = document.getElementById('detectedAddressText');
                    if (detectedText) detectedText.textContent = window.currentGPSAddress.full;
                    const completeModal = document.getElementById('completeLocationModal');
                    if (completeModal) completeModal.classList.add('active');
                } else if (retries > 15) {
                    clearInterval(checkGPS);
                    currentLocationText.textContent = 'Could not detect location. Please add manually.';
                }
            }, 500);
        }
    });
    ...
    completeLocationForm?.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        if (user.firebaseUid) {
            try {
                await fetch('/api/user/addresses', {
                    ...
                    body: JSON.stringify({ uid: user.firebaseUid, address: newAddress })
                });
    ...
    function renderSavedAddresses() {
        ...
        savedAddresses.forEach(address => {
            const addrId = String(address._id || address.id);
            const card = document.createElement('div');
            card.className = 'saved-address-card';
            const landmarkText = address.landmark ? `, ${address.landmark}` : '';

            // Handle both home-created (line1/line2) and cart-created (apartment/street) addresses
            const line1Str = [address.doorNo, address.apartment, address.street || address.line1].filter(Boolean).join(', ');
            const line2Str = [address.line2, address.city].filter(Boolean).join(', ');
            const addressText = [line1Str, line2Str].filter(Boolean).join(', ');

            card.innerHTML = `
                <div class="address-icon">${address.label === 'Home' ? '🏠' : address.label === 'Work' ? '💼' : '📍'}</div>
                <div class="address-details">
                    <div class="address-header">
                        <span class="address-label">${address.label || 'Home'}</span>
                    </div>
                    <div class="address-text">${addressText}${landmarkText}${address.pincode ? ' - ' + address.pincode : ''}</div>
                    <div class="address-phone">Phone: ${address.phone || 'N/A'}</div>

                    <div class="address-actions">
                        <button class="address-action-btn delete-btn" data-id="${addrId}">
            ...
            card.addEventListener('click', (e) => {
                ...
                const shortAddress = [address.line2 || address.city, address.city].filter(Boolean)[0] || address.label;
            ...
            card.querySelector('.delete-btn')?.addEventListener('click', async (e) => {
                ...
                // Try to delete from DB first if user is logged in
                const user = JSON.parse(localStorage.getItem('savourlyUser') || '{}');
                if (user.firebaseUid) {
                    try {
                        await fetch('/api/user/addresses', {
                            method: 'DELETE',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ uid: user.firebaseUid, addressId: deleteId })
                        });
                    } catch (err) {
                ...
                savedAddresses = savedAddresses.filter(a => String(a._id || a.id) !== deleteId);
                localStorage.setItem('savourlyAddresses', JSON.stringify(savedAddresses));
                renderSavedAddresses();
            });
    ...
    // Open Cart Page instead of modal
    ...
    payOnlineBtn?.addEventListener('click', async () => {
        const user = JSON.parse(localStorage.getItem('savourlyUser')) || { id: 'guest', email: 'guest@example.com' };

        try {
            payOnlineBtn.innerHTML = 'Processing...';
            ...
            const options = {
                ...
                name: "Savourly",
                description: "Premium Meal Plan Subscription",
                order_id: order.id,
            ...
            rzp1.on('payment.failed', function (response) {
                ...
                payOnlineBtn.innerHTML = `...reset button content...`; // Simplified for brevity, usually reset logic here
                payOnlineBtn.disabled = false;
            });
            rzp1.open();
    ...
    codBtn?.addEventListener('click', async () => {
        // Simply redirect to success or create order with 'cod' status logic
    ...
    /* 
    // Commented out old checkout logic
    ...
    function updateAuthUI(user) {
        ...
        if (user) {
            ...
            const avatarHTML = photoURL
                ...
                : `<span style="width:32px;height:32px;border-radius:50%;background:#00C9A7;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;">${initial}</span>`;

            // Unified Desktop & Mobile: avatar + static 'Profile' text (text hidden on mobile via CSS)
            desktopAuthBtn.innerHTML = `<a href="/profile" class="btn btn-outline" style="display:flex;align-items:center;gap:8px;padding:6px 16px;border-radius:24px;">
                ${avatarHTML}<span class="hide-on-mobile">Profile</span>
            </a>`;
        } else {
            // Logged out
            desktopAuthBtn.innerHTML = `<a href="/login" class="btn btn-outline">Login</a>`;
        }
    }

    // Check localStorage first for instant UI (no flicker)
    const cachedUser = localStorage.getItem('savourlyUser');
    if (cachedUser) {
        try {
            const u = JSON.parse(cachedUser);
            updateAuthUI({ displayName: u.name, email: u.email, photoURL: u.profilePic });
        } catch { /* ignore */ }
    }

    // Then use Firebase auth state for truth
        ...
        if (typeof firebase !== 'undefined' && firebase.apps && firebase.apps.length > 0) {
            firebase.auth().onAuthStateChanged(function (user) {
                ...
                if (user) {
                    ...
                    const stored = JSON.parse(localStorage.getItem('savourlyUser') || '{}');
                    if (stored.firebaseUid !== user.uid) {
                        localStorage.setItem('savourlyUser', JSON.stringify({
                            firebaseUid: user.uid, email: user.email,
                            name: user.displayName || user.email?.split('@')[0],
                            profilePic: user.photoURL || '', role: stored.role || 'user'
                        }));
                    }
                } else {
